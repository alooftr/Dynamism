...
Vector player_t::get_bone_pos(int iBone)
{
...
int weapon_t::get_weapon_group(bool rage)
{
	...
	{
		if (is_pistol() || m_iItemDefinitionIndex() == WEAPON_DEAGLE)
			return 1;
		else if (is_rifle())
			return 2;
		else if (is_smg())
...
bool& weapon_t::IsCustomMaterialInitialized()
{
	static auto uAddress = *reinterpret_cast<std::uintptr_t*>(util::FindSignature("client.dll", "C6 86 ? ? ? ? ? FF 50 04") + 0x2);
	return *reinterpret_cast<bool*>(reinterpret_cast<std::uintptr_t>(this) + uAddress);
}

CUtlVector<IRefCounted*>& weapon_t::GetCustomMaterials()
{
	static auto uAddress = *reinterpret_cast<std::uintptr_t*>(util::FindSignature("client.dll", "83 BE ? ? ? ? ? 7F 67") + 0x2) - 0xC;
...
AnimationLayer* player_t::get_animlayers(int idx)
{
	if (idx < 15)
		return &get_animlayers()[idx];
	else
		return nullptr;
}
