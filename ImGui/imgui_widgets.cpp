...
bool ImGui::KeybindButton(const char* label, const char* unique_id, const ImVec2& size_arg, bool clicked, ImGuiButtonFlags flags)
{
    ...
    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(ImVec4(14 / 255.f, 246 / 255.f, 250 / 255.f, it_hover->second)), 2.f * c_menu::get().dpi_scale);
...
bool ImGui::CustomButton(const char* label, const char* label_id, const ImVec2& size_arg, bool use_icon, ImFont* icon_font, const char* icon_label)
{
    ...
    const ImVec4 text_act = ImVec4(255 / 255.f, 255 / 255.f, 255 / 255.f, 1.f);
    const ImVec4 icon_act = ImVec4(14 / 255.f, 246 / 255.f, 250 / 255.f, 1.f);

    RenderFrame(bb.Min, bb.Max, col, true, 2.f * c_menu::get().dpi_scale);

    window->DrawList->PushClipRect(bb.Min, bb.Max, false);
    ImGui::GetWindowDrawList()->AddCircleFilled(it_circle->second.pos, it_circle->second.radius, GetColorU32(ImVec4(14 / 255.f, 246 / 255.f, 250 / 255.f, it_circle->second.alpha)), 60);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawCornerFlags rounding_corners)
{
    ...
    // Render
    ImVec4 hover_dis = ImVec4(14 / 255.f, 206 / 255.f, 250 / 255.f, 1.f);
    ImVec4 hover_act = ImVec4(14 / 255.f, 246 / 255.f, 250 / 255.f, 1.f);
...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec2& padding, const ImVec4& bg_col, const ImVec4& tint_col, bool preview, const ImVec4& skin_rarity)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    RenderFrame(bb.Min, bb.Max, col, true, ImClamp((float)ImMin(padding.x, padding.y), 0.0f, g.Style.FrameRounding));
     ...
     if (preview)
     {
         window->DrawList->AddRectFilled(bb.Min, ImVec2(bb.Min.x + 3, bb.Max.y), GetColorU32(skin_rarity));
     }
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, bool preview, const ImVec4& skin_rarity, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
    ...
    return ImageButtonEx(id, user_texture_id, size, uv0, uv1, padding, bg_col, tint_col, preview, skin_rarity);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImU32 col_bg;
    if (!hovered)
        col_bg = ImGui::GetColorU32(ImLerp(ImVec4(202 / 255.f, 202 / 255.f, 202 / 255.f, 255.f / 255.f), ImVec4(14 / 255.f, 246 / 255.f, 250 / 255.f, 1.00f), t));
    if (hovered)
        col_bg = ImGui::GetColorU32(ImLerp(ImVec4(202 / 255.f, 202 / 255.f, 202 / 255.f, 1.f), ImVec4(14 / 255.f, 246 / 255.f, 250 / 255.f, 0.7f), t));


    const float check_sz = ImMin(check_bb.GetWidth(), check_bb.GetHeight());
    const float pad = ImMax(1.0f, (float)(int)(check_sz / 6.0f));

    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, GetColorU32(ImVec4(0 / 255.f, 0 / 255.f, 0 / 255.f, 0.1f)), 2.f);
    window->DrawList->AddRectFilled(check_bb.Min , check_bb.Max, GetColorU32(config_system.g_cfg.menu.menu_theme == 1 ? ImVec4(21 / 255.f, 22 / 255.f, 26 / 255.f, 1.00f) : ImVec4(27 / 255.f, 31 / 255.f, 42 / 255.f, 1.00f)), 2.f);

    if (*v) {
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, col_bg, 2.f); // Main gradient.
        window->DrawList->AddRectFilledMultiColor(check_bb.Min, check_bb.Max, GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.05f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.05f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.38f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.38f))); // Main gradient.
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int* current_item, bool is_keyboard)
{
    ImGuiContext& g = *GImGui;
    ...
    if (!is_keyboard)
        pressed = ButtonBehavior(ImRect(ImVec2(185, 0) + frame_bb.Min, ImVec2(220, 0) + frame_bb.Max), id, &hovered, &held);

    if (is_keyboard)
        pressed = ButtonBehavior(ImRect(ImVec2(85, 0) + frame_bb.Min,ImVec2(120, 0) + frame_bb.Max), id, &hovered, &held);
    ...
    ImU32 col_bg2;
    col_bg2 = ImGui::GetColorU32(ImLerp(ImVec4(145 / 255.f, 144 / 255.f, 148 / 255.f, 255.f / 255.f), ImVec4(235 / 255.f, 235 / 255.f, 235 / 255.f, 255.f / 255.f), t));
    ...
    if (popup_open || ImGui::IsItemHovered())
        PushStyleColor(ImGuiCol_Text, col_bg);

    
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview) && !is_keyboard)
        RenderText(ImVec2(frame_bb.Min.x + 190.f, frame_bb.Min.y + style.FramePadding.y), preview_value);

   if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview) && is_keyboard)
        RenderText(ImVec2(frame_bb.Min.x + 90.f, frame_bb.Min.y + style.FramePadding.y), preview_value);

    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        if(!is_keyboard)
        RenderArrow(ImVec2(frame_bb.Max.x + 85.f - arrow_size + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y), popup_open ? ImGuiDir_Down : ImGuiDir_Left);
        if(is_keyboard)
         RenderArrow(ImVec2(frame_bb.Max.x - 30 - arrow_size + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y), popup_open ? ImGuiDir_Down : ImGuiDir_Left);
    ...
    {
        g.NextWindowData.Flags |= ImGuiNextWindowDataFlags_HasSizeConstraint;
        g.NextWindowData.SizeConstraintRect.Min.x = ImMax(g.NextWindowData.SizeConstraintRect.Min.x - 100.f, w);
    }
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(w - 100, 0.0f), ImVec2(120.f, ((CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)) * it_open->second)));
    ...
    if (ImGuiWindow* popup_window = FindWindowByName(name))
        if (popup_window->WasActive)
        {
            ...
            ImVec2 pos;
            if (!is_keyboard)
                pos = FindBestWindowPosForPopupEx(frame_bb.GetBL(), size_expected, &popup_window->AutoPosLastDirection, r_outer, ImRect(frame_bb.Min + ImVec2(185, 0), frame_bb.Max), ImGuiPopupPositionPolicy_ComboBox);
            if (is_keyboard)
                pos = FindBestWindowPosForPopupEx(frame_bb.GetBL(), size_expected, &popup_window->AutoPosLastDirection, r_outer, ImRect(frame_bb.Min + ImVec2(85, 0), frame_bb.Max), ImGuiPopupPositionPolicy_ComboBox);
...
bool ImGui::Combo(const char* label, int* current_item, const char* const items[], int items_count, int height_in_items, bool isKeyboard)
{
    const bool value_changed = Combo(label, current_item, Items_ArrayGetter, (void*)items, items_count, height_in_items, isKeyboard);
    return value_changed;
}

// Combo box helper allowing to pass all items in a single string literal holding multiple zero-terminated items "item1\0item2\0"
bool ImGui::Combo(const char* label, int* current_item, const char* items_separated_by_zeros, int height_in_items, bool isKeyboard)
{
    int items_count = 0;
    ...
    bool value_changed = Combo(label, current_item, Items_SingleStringGetter, (void*)items_separated_by_zeros, items_count, height_in_items, isKeyboard);
    return value_changed;
}

bool ImGui::Combo(const char* label, int* current_item, bool(*items_getter)(void* data, int idx, const char** out_text), void* data, int items_count, int popup_max_height_in_items, bool isKeyboard)
{
    ImGuiContext& g = *GImGui;
    ...
    if (!BeginCombo(label, preview_value, ImGuiComboFlags_None, current_item, isKeyboard))
    {
        return false;
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, bool minimum_damage, float power)
{
 ...
 // Draw frame
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    window->DrawList->AddRectFilled(frame_bb.Min + ImVec2{ 0,7 }, frame_bb.Max - ImVec2{ 12,3 }, ImColor(17, 17, 17, 90), 6.f);
    ...
    col_bg = ImGui::GetColorU32(ImLerp(ImVec4(14 / 255.f, 246 / 255.f, 250 / 255.f, 255.f / 255.f), ImVec4(14 / 255.f, 246 / 255.f, 250 / 255.f, 255.f / 255.f), t));
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImVec2 labelsize = CalcTextSize(buf, NULL, true);
    ...
    if (!is_multiline)
    {
        ...
        float t = is_multiline ? 0.0f : 1.0f;
        ...
        {
            float t_anim = ImSaturate(g.LastActiveIdTimer / ANIM_SPEED);
            t = is_multiline ? (t_anim) : (1.0f - t_anim);
        }

        ImU32 col_bg;
        col_bg = ImGui::GetColorU32(ImLerp(ImVec4(14 / 255.f, 246 / 255.f, 250 / 255.f, 1.f), config_system.g_cfg.menu.menu_theme == 1 ? ImVec4(21 / 255.f, 22 / 255.f, 26 / 255.f, 1.00f) : ImVec4(27 / 255.f, 31 / 255.f, 42 / 255.f, 1.00f), t));

        ImGui::GetWindowDrawList()->AddRect(frame_bb.Min, frame_bb.Max, col_bg, 3.f);
    }
    ...
    if (label_size.x > 0 && !render_cursor)
        ImGui::GetWindowDrawList()->AddText(ImVec2(frame_bb.Min.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y), ImColor(255,255,255,100), labelsize.x > 0 ? "" : label);
...
bool ImGui::BlssButtonXD(const char* label, const char* icon, ImFont* icon_font, const ImVec2& size_arg, bool inuse, ImGuiWindowFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (icon)
    {
        ...
        PushFont(icon_font);
        const ImVec2 icon_size = CalcTextSize(icon, NULL, true);
        RenderText(pos + ImVec2(size_arg.x / 2 - icon_size.x / 2, 40), icon);
        ImGui::PopStyleColor();
        PopFont();
    }
...
bool ImGui::BlssButton(const char* label, const char* icon, ImFont* icon_font, bool inuse, const ImVec2& size_arg)
{
    return BlssButtonXD(label, icon, icon_font, size_arg, inuse, 0);
}
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    if (size.y == 0.0f)
        size.y = 10;
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + size);
    ItemSize(bb, (size.y >= default_size) ? 0.0f : 0.0f);
    if (!ItemAdd(bb, id))
        return false;
    ...
    else
    {
        ...
        if (col_source.w < 1.0f)
            ...
            window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), 2.f, ImDrawCornerFlags_All);
    }
    RenderNavHighlight(bb, id);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    PopStyleColor();
    if (selected)
        RenderCheckMark(window->DrawList, pos + ImVec2(window->DC.MenuColumns.Pos[2] + extra_w + g.FontSize * 0.40f, g.FontSize * 0.134f * 0.5f), GetColorU32(ImGuiCol_Text), g.FontSize * 0.866f);
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled)
{
    ...
    ImGuiSelectableFlags flags = ImGuiSelectableFlags_SelectOnRelease | ImGuiSelectableFlags_DontClosePopups | ImGuiItemFlags_SelectableDontClosePopup | ImGuiSelectableFlags_SetNavIdOnHover | (enabled ? 0 : ImGuiSelectableFlags_Disabled);
    bool pressed;
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
