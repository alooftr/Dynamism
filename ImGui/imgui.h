...
namespace ImGui
{
    ...
    IMGUI_API const char*   GetVersion();                               // get the compiled version string e.g. "1.23" (essentially the compiled value for IMGUI_VERSION)
       ...
       //Others
    IMGUI_API bool          BlssButtonXD(const char* label, const char* icon, ImFont* icon_font, const ImVec2& size_arg, bool inuse, ImGuiWindowFlags flags);
    IMGUI_API bool          BlssButton(const char* label, const char* icon, ImFont* icon_font, bool inuse, const ImVec2& size_arg);
    ...
    IMGUI_API bool          BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags = 0, int* current_item = nullptr, bool is_keyboard = false);
    IMGUI_API void          EndCombo(); // only call EndCombo() if BeginCombo() returns true!
    IMGUI_API bool          Combo(const char* label, int* current_item, const char* const items[], int items_count, int popup_max_height_in_items = -1, bool isKeyboard = false);
    IMGUI_API bool          Combo(const char* label, int* current_item, const char* items_separated_by_zeros, int popup_max_height_in_items = -1, bool isKeyboard = false);      // Separate items with \0 within a string, end item-list with \0\0. e.g. "One\0Two\0Three\0"
    IMGUI_API bool          Combo(const char* label, int* current_item, bool(*items_getter)(void* data, int idx, const char** out_text), void* data, int items_count, int popup_max_height_in_items = -1, bool isKeyboard = false);
