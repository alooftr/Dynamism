...
void legit_bot::do_smooth(const Vector& currentAngle, const Vector& aimAngle, Vector& angle)
{
	...
	aim_punch.x *= (config_system.g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].rcs / 100.f);
	aim_punch.y *= (config_system.g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].custom_rcs_smooth / 100.f);
...
void legit_bot::createmove(CUserCmd* cmd)
{
	...
	if (!config_system.g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].silent)
...
void legit_bot::draw_fov()
{
	...
	if (!config_system.g_cfg.legitbot.draw_fov)
		return;

	int x, y;
	m_engine()->GetScreenSize(x,y);
	float radius = config_system.g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].fov / 90.f * x / 2;

	if (g_ctx.globals.weapon->is_knife() || g_ctx.globals.weapon->is_grenade())
		return;

	g_Render->DrawCircle(x /2, y / 2, radius, 64, Color(255,255,255,100));
...
void legit_bot::find_best_point(CUserCmd* cmd, float fov_v)
{
	...
	switch (config_system.g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].autofire_hitchance)
	{
	case 0:
		hitbox = HITBOX_HEAD;
		break;
	...
	else if (config_system.g_cfg.legitbot.weapon[g_ctx.globals.current_weapon].priority == 1) // head only
	{
		...
		auto bone_pos = target->hitbox_position(hitbox);
		math::vector_angles(bone_pos - g_ctx.globals.eye_pos, ang);
		math::normalize_angles(ang);
		distance = g_ctx.globals.eye_pos.DistTo(bone_pos);
