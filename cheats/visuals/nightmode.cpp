...
void nightmode::modulate(MaterialHandle_t i, IMaterial* material, bool backup = false) 
{
	...
	if (strstr(name, crypt_str("World")))
	{
		...
		material->AlphaModulate((float)config_system.g_cfg.esp.worlds_color.a() / 255.0f);
		material->ColorModulate((float)config_system.g_cfg.esp.worlds_color.r() / 255.0f, (float)config_system.g_cfg.esp.worlds_color.g() / 255.0f, (float)config_system.g_cfg.esp.worlds_color.b() / 255.0f);
	}
	else if (strstr(name, crypt_str("StaticProp")))
	{
		...
		material->AlphaModulate((float)config_system.g_cfg.esp.pops_color.a() / 255.0f);
		material->ColorModulate((float)config_system.g_cfg.esp.pops_color.r() / 255.0f, (float)config_system.g_cfg.esp.pops_color.g() / 255.0f, (float)config_system.g_cfg.esp.pops_color.b() / 255.0f);
	}
}
