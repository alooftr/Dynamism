...
{
	static auto original_fn = clientmode_hook3.GetOriginal <OverrideView_t> ();
	...
	if (g_ctx.local())
	{
		...
		if (weapon)
		{
			...
			else if (config_system.g_cfg.esp.REMOVALS_ZOOM && config_system.g_cfg.player.enable)
			{
...
void thirdperson(bool fakeducking)
{
	...
	ConVar* sv_cheats = m_cvar()->FindVar("sv_cheats");
	sv_cheats->iSetValue(1);
