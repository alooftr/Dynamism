...
void remove_smoke()
{
	if (config_system.g_cfg.player.enable && config_system.g_cfg.esp.REMOVALS_SMOKE)
	{
		static auto smoke_count = *reinterpret_cast<uint32_t**>(util::FindSignature(crypt_str("client.dll"), crypt_str("A3 ? ? ? ? 57 8B CB")) + 0x1);
		*(int*)smoke_count = 0;
	}

	if (g_ctx.globals.should_remove_smoke == config_system.g_cfg.player.enable && config_system.g_cfg.esp.REMOVALS_SMOKE)
		return;

	g_ctx.globals.should_remove_smoke = config_system.g_cfg.player.enable && config_system.g_cfg.esp.REMOVALS_SMOKE;
...
{
	static auto original_fn = client_hook.GetOriginal <FrameStageNotify_t>();
	...
	if (stage == FRAME_RENDER_START)
	{
		...
		if (config_system.g_cfg.esp.REMOVALS_FLASH && g_ctx.local()->m_flFlashDuration() && config_system.g_cfg.player.enable) //-V807
			g_ctx.local()->m_flFlashDuration() = 0.0f;

		if (*(bool*)m_postprocessing() != (config_system.g_cfg.player.enable && config_system.g_cfg.esp.REMOVALS_POSTPROCESSING && (!config_system.g_cfg.esp.world_modulation || !config_system.g_cfg.esp.exposure)))
			*(bool*)m_postprocessing() = config_system.g_cfg.player.enable && config_system.g_cfg.esp.REMOVALS_POSTPROCESSING && (!config_system.g_cfg.esp.world_modulation || !config_system.g_cfg.esp.exposure);

		if (config_system.g_cfg.esp.REMOVALS_RECOIL && config_system.g_cfg.player.enable)
		{
		...
		if (g_ctx.local()->is_alive())
		{
			...
			if (config_system.g_cfg.player.enable && config_system.g_cfg.esp.REMOVALS_SCOPE)
			{
	...
	if (stage == FRAME_RENDER_END)
	{
		...
		if (config_system.g_cfg.player.enable && config_system.g_cfg.esp.REMOVALS_ZOOM && config_system.g_cfg.esp.fix_zoom_sensivity && zoom_sensitivity_ratio_mouse->GetFloat() == 1.0f) //-V550
			zoom_sensitivity_ratio_mouse->SetValue(0.0f);
		else if ((!config_system.g_cfg.player.enable || !config_system.g_cfg.esp.REMOVALS_ZOOM || !config_system.g_cfg.esp.fix_zoom_sensivity) && !zoom_sensitivity_ratio_mouse->GetFloat())
		...
		if (current_shot != g_ctx.shots.end())
		{
			if (!current_shot->latency)
			{
				...
				if (!current_shot->hurt_player)
				{
					...
					else if (config_system.g_cfg.misc.events_to_log)
					{
	...
	if ( config_system.g_cfg.esp.REMOVALS_RECOIL && aim_punch && view_punch && g_ctx.local()->is_alive() )
	{
	...
	if (g_ctx.local()->is_alive() && death_notice == nullptr)
	{
		if (!death_notice)
