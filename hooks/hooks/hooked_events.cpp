...
void C_HookedEvents::FireGameEvent(IGameEvent* event)
{
	...
	if (!strcmp(event->GetName(), crypt_str("player_footstep")))
	{
		...
		if (e->valid(false, false))
		{
			...
			if (config_system.g_cfg.player.type[type].footsteps && !e->IsDormant())
			{
				...
				beam_info.m_nModelIndex = model_index;
				...
				beam_info.m_flRed = (float)config_system.g_cfg.player.type[type].footsteps_color.r();
				beam_info.m_flGreen = (float)config_system.g_cfg.player.type[type].footsteps_color.g();
				beam_info.m_flBlue = (float)config_system.g_cfg.player.type[type].footsteps_color.b();
				beam_info.m_flBrightness = (float)255.f;
				...
				beam_info.m_vecCenter = e->GetAbsOrigin() + Vector(0.0f, 0.0f, 5.0f);
				beam_info.m_flStartRadius = 1.f;
				beam_info.m_flEndRadius = 150.f;

				auto beam_draw = m_viewrenderbeams()->CreateBeamRingPoint(beam_info);
	...
	else if (!strcmp(event_name, crypt_str("player_hurt")))
	{
		...
		if (attacker_id == m_engine()->GetLocalPlayer() && user_id != m_engine()->GetLocalPlayer())
		{
			if (config_system.g_cfg.esp.hit)
			{
				...
				{
				case 1:
					m_surface()->PlaySound_("buttons/arena_switch_press_02.wav");
					break;
				case 2:
					m_surface()->PlaySound_("weapons/scar20/scar20_unsil-1.wav");
					break;
				case 3:
					m_surface()->PlaySound_("weapons/smokegrenade/grenade_hit1.wav");
					break;
				case 4:
					m_surface()->PlaySound_(config_system.g_cfg.esp.hitfile.c_str());
					break;
