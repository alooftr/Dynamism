...
#include "../../cheats/legitbot/Trigger.h"
...
{
	static auto original_fn = clientmode_hook.GetOriginal <CreateMove_t>();
	...
	g_ctx.globals.exploits = g_ctx.globals.weapon->can_double_tap() && misc::get().double_tap_key || misc::get().hide_shots_key;
	...
	if (config_system.g_cfg.misc.bunnyhop)
		g_Movement.BunnyHop(m_pcmd);

	misc::get().SlideWalk(m_pcmd);
	CGrenadePrediction::get().trace(m_pcmd);
	misc::get().AutoCrouch(m_pcmd);

	if (config_system.g_cfg.misc.airstrafe > 0)
		g_Movement.AirStrafe(m_pcmd, wish_angle);
	...
	if (config_system.g_cfg.misc.edgebug && key_binds::get().get_key_bind_state(24) && engineprediction::get().backup_data.flags & FL_ONGROUND && !(g_ctx.local()->m_fFlags() & FL_ONGROUND)) //-V807
	{
		m_pcmd->m_buttons |= IN_DUCK;
	}
	if (config_system.g_cfg.misc.edge && key_binds::get().get_key_bind_state(19) && engineprediction::get().backup_data.flags & FL_ONGROUND && !(g_ctx.local()->m_fFlags() & FL_ONGROUND)) //-V807
	{
		m_pcmd->m_buttons |= IN_JUMP;

		if (config_system.g_cfg.misc.duckinair)
			m_pcmd->m_buttons |= IN_DUCK;
	}
	...
	TriggerBot.TriggerBot(m_pcmd);
	g_MovementRecorder.MovementR(m_pcmd);
	g_MovementRecorder.MovementP(m_pcmd);
	g_Movement.jBug(m_pcmd);
	g_Movement.LJump(m_pcmd);
	...
	if (config_system.g_cfg.misc.buybot_enable && g_ctx.globals.should_buy)
	{
		...
		if (!g_ctx.globals.should_buy)
		{
			...
			if (config_system.g_cfg.misc.vest)
				buy += crypt_str("buy vest; ");

			if (config_system.g_cfg.misc.helm)
				buy += "buy vesthelm; ";

			if (config_system.g_cfg.misc.zeus)
				buy += crypt_str("buy taser; ");

			if (config_system.g_cfg.misc.inc)
				buy += crypt_str("buy molotov; ");

			if (config_system.g_cfg.misc.smoke)
				buy += "buy smokegrenade; ";

			if (config_system.g_cfg.misc.he)
				buy += "buy hegrenade; ";

			if (config_system.g_cfg.misc.defuse)
